...
## Regras de Sincronização — Permissões CRM

### Sidebar ↔ crmModules.ts (OBRIGATÓRIO)

Sempre que adicionar ou remover um item do sidebar em `src/components/layout/AppSidebar.tsx`:

1. **Se o item tem `moduleKey != null`** → adicionar a mesma key em `src/lib/crmModules.ts` na seção correspondente.
2. **Se o item não tem `moduleKey`** (moduleKey: null) e deve ser controlado por permissão → atribuir uma key e adicionar em crmModules.ts.
3. **Nunca usar `moduleKey: null`** para funcionalidades que devem ter controle de acesso por usuário.

**Mapeamento de seções:**
- `mainNav` → seção `"Principal"`
- `operationsNav` → seção `"Operações"`
- `managementNav` → seção `"Gestão"`
- `otherNav` → seção `"Outros"`

**Checklist ao criar nova rota/página:**
- [ ] Rota adicionada em `src/App.tsx`
- [ ] Item adicionado no array correto de `AppSidebar.tsx` com `moduleKey`
- [ ] Entry adicionada em `src/lib/crmModules.ts` com a mesma key
- [ ] Admin liberou acesso em `/crm-permissions` para os usuários corretos
