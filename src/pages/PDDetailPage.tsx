...
const PDDetailPage = () => {
  ...
  const { data: sr, isLoading } = useQuery({
    queryKey: ["service_request_detail_pd", id],
    queryFn: async () => {
  ...
  const quoteClientId = (linkedQuote as any)?.client_id as string | undefined;
  const hasTicketClient = !!sr?.tickets?.clients;
  const { data: quoteClient } = useQuery({
    queryKey: ["quote_client_for_contract", quoteClientId],
    enabled: !!quoteClientId && !hasTicketClient,
    queryFn: async () => {
      const { data, error } = await supabase
        .from("clients")
        .select("name, contact_person, document, email, phone, whatsapp, address, city, state, zip_code")
        .eq("id", quoteClientId!)
        .single();
      if (error) throw error;
      return data;
  ...
  const buildPdExcelPayload = (): ExportDocument => {
    const p = buildPdPdfPayload();
    return {
  ...
  const handleExportPdf = () => { generateQuotePdf(buildPdPdfPayload()).save(`${requestNumber}.pdf`); };
  const handleExportExcel = () => exportDocumentToExcel(buildPdExcelPayload());
  const handlePrint = () => printPdf(generateQuotePdf(buildPdPdfPayload()));

  const ensureLinkedQuote = async (): Promise<string | null> => {
    if (linkedQuote) return linkedQuote.id;
    const clientId = (sr as any)?.tickets?.client_id;
    if (!clientId) { toast.error("Ticket sem cliente vinculado"); return null; }
    const { data: newQuote, error } = await supabase
      .from("quotes")
      .insert({ service_request_id: id!, client_id: clientId, equipment_id: (sr as any)?.tickets?.equipment_id || null })
      .select()
      .single();
  ...
  const handleSaveSquadNotes = async () => {
    ...
    try {
      ...
      if (error) throw error;
      await notifySquad({ recordType: "pd", recordId: id!, reference: requestNumber });
      toast.success("Observações salvas e enviadas ao Squad!");
      qc.invalidateQueries({ queryKey: ["service_request_detail_pd", id] });
    } catch (err: any) {
  ...
  const handleStatusChange = async (val: string) => {
    ...
    else { toast.success("Status atualizado"); qc.invalidateQueries({ queryKey: ["service_request_detail_pd", id] }); }
  };
    ...
    try {
      ...
      const itensPedido = await Promise.all(erpRows.map(async (row, idx) => {
        ...
        return {
          ...
          valorUnitario: parseFloat(Number(erpData?.valorUnitario || 0).toFixed(4)),
      ...
      const codigoPedido = nomusFields.pedido || requestNumber;
      ...
      let existingNomusId = (sr as any).nomus_order_id as number | null;
      ...
      else toast.success("PD aprovada. Pedido já existia no Nomus — verifique lá se precisar editar.");
      qc.invalidateQueries({ queryKey: ["service_request_detail_pd", id] });
    } catch (err: any) {
  ...
  return (
    <div className="max-w-6xl mx-auto">
          ...
          <div className="flex items-center gap-3 mt-4 pt-3 border-t flex-wrap">
            ...
            {totals.charged > 0 && (
              <span className="text-sm text-muted-foreground">
                = {installments}x de{" "}
                <strong className="text-foreground font-mono">R$ {(totals.charged / installments).toFixed(2)}</strong>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="md:col-span-2">
          ...
          <Textarea
            ...
            placeholder="Observações do pedido de venda..."
            rows={3}
          />
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div>
          <label className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold mb-1 block">Status do PD</label>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="bg-card rounded-xl border shadow-sm overflow-hidden mb-6">
        ...
        <div className="p-4 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <CreateNomusClientDialog
              ...
              crmClient={sr?.tickets?.clients as any}
        ...
        <div className="px-4 pb-4">
          <Button className="gap-1.5 w-full md:w-auto" onClick={handleApprove} disabled={approving}>
            ...
            {approving
              ? ((sr as any).nomus_order_id ? "Atualizando no ERP..." : "Criando no ERP...")
              : ((sr as any).nomus_order_id ? "Atualizar Pedido no Nomus" : "Criar Pedido no Nomus")}
          </Button>
        </div>
      </motion.div>
      ...
      <div className="flex flex-wrap gap-2 border-t pt-4">
        <Button size="sm" variant="outline" className="gap-1.5" onClick={() => {
          ...
          <ArrowLeft className="h-3.5 w-3.5" /> {fromTicketId ? "Voltar ao Card" : "Voltar para Pedidos de Venda"}
        </Button>
      </div>
    </div>
  );
