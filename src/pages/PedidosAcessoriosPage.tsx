...
const PedidosAcessoriosPage = () => {
  const navigate = useNavigate();

  const { data: requests, isLoading } = useQuery({
    queryKey: ["service_requests_pa"],
    queryFn: async () => {
  ...
  return (
    <div>
      <PageHeader
        title="Pedidos de Acessórios (PA)"
        description="Pedidos de peças e acessórios"
        icon={ShoppingBag}
        action={
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
        {isLoading ? (
          ...
          <div className="p-8 text-center text-muted-foreground text-sm bg-card rounded-xl border">Nenhum pedido de acessório registrado.</div>
        ) : (
          requests.map((sr: any) => (
            <div
              key={sr.id}
              className="border rounded-lg p-4 bg-card hover:bg-muted/30 transition-colors cursor-pointer"
              onClick={() => navigate(`/pedidos-acessorios/${sr.id}`)}
            >
...
export default PedidosAcessoriosPage;
