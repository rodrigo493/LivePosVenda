...
-- Update quote number format: OC.ANO.NUM starting at 828
...
BEGIN
  ...
  NEW.quote_number := 'OC.' || EXTRACT(YEAR FROM now())::text || '.' || LPAD(seq_num::TEXT, 3, '0');
  RETURN NEW;
END;
$$;

-- Create PA number generator function
...
BEGIN
  ...
  RETURN 'PA.' || EXTRACT(YEAR FROM now())::text || '.' || LPAD(seq_num::TEXT, 3, '0');
END;
$$;

-- Create PG number generator function
...
BEGIN
  ...
  RETURN 'PG.' || EXTRACT(YEAR FROM now())::text || '.' || LPAD(seq_num::TEXT, 3, '0');
END;
$$;

GRANT EXECUTE ON FUNCTION public.generate_pa_number() TO authenticated;
GRANT EXECUTE ON FUNCTION public.generate_pg_number() TO authenticated;
