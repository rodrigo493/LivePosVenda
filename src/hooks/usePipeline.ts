  ...
  return useQuery({
    ...
    queryFn: async () => {
      let q = (supabase as any)
        .from("tickets")
        .select("*, clients(name), equipments(serial_number, equipment_models(name)), quotes(id, quote_number, status, total, service_request_id, warranty_claim_id, updated_at, service_requests(document_type, contract_generated_at)), tasks(id, title, due_date, due_time, status)")
